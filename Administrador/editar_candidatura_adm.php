...
<div class="main-wrapper">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        $ID_candidatura = filter_input(INPUT_GET, 'ID_candidatura');
        $operacao = filter_input(INPUT_GET, 'operacao');

        /* validar os dados recebidos através do pedido */
        if (empty($ID_candidatura) && $operacao!="editar"){
    ...
    /* texto sql da sql*/
    $editar = "SELECT candidatura.*, oferta.*, estado_candidatura.* FROM candidatura inner join oferta on candidatura.Oferta_ID_oferta = oferta.ID_oferta inner join estado_candidatura on candidatura.Estado_candidatura_ID_estado = estado_candidatura.ID_estado WHERE ID_candidatura = '$ID_candidatura' ";

//SELECT  candidatura.*, oferta.*, estado_candidatura.* FROM candidatura  inner join oferta on candidatura.Oferta_ID_oferta = oferta.ID_oferta inner join estado_candidatura on candidatura.Estado_candidatura_ID_estado= estado_candidatura.ID_estado  WHERE  ID_candidatura IN (".$idyx.")";
        ...
        <div class="content">
            <div class="row">
                <div class="col-sm-7 col-6">
                    <h4 class="page-title">Alteração de estado da candidatura </h4>
                </div>
                <form action="edit_candidatura_adm.php" method="post" >
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-25">
                                                            <div class="form-group">
                                                                <label>Estado da candidatura <span class="text-danger"></span></label>
                                                                <select class="custom-select d-block w-100"  name="ca_Estado_candidatura_ID_estado" >

                                                                    <option value=<?=$row['ID_estado']?> selected><?= $row['Estado_Estado']?></option>
                                                                    <?php
                                                                    require("../config.php");
                                                                    $link->set_charset("utf8");
                                                                    $tp=$row['ID_estado'];
                                                                    $consulta = "SELECT * FROM estado_candidatura where ID_estado!=$tp";
                                                                    ...
                                                                    if (!$resultado = $link->query($consulta)) {
                                                                        ...
                                                                        while ($rowss = $resultado->fetch_assoc()) {
                                                                            ...
                                                                            ?>
                                                                            <option value=<?= $rowss['ID_estado']?>><?= $rowss['Estado_Estado']?></option>
                                                                            <?php
                                                                        }
                                                        ...
                                                        <div class="col-sm-9">
                                                            <div class="form-group">
                                                                <input name="ca_ID_candidatura" class="form-control" type="text" value="<?=$row['ID_candidatura']?>">
                                                            </div>
                                                        </div>
