...
$username =  $Password  = $Nome = $Nif = $Bi = $Telefone = $Localidade = $Morada = $Codigo_Postal = $Email = $confirm_password = "";
$username_err = $Password_err = $Nome_err = $Nif_err = $Bi_err = $Telefone_err = $Localidade_err = $Morada_err = $Codigo_Postal_err = $Email_err = $confirm_password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Validate name
    if(empty(trim($_POST["Bi"]))){
        $Bi_err = "Por favor coloque o numero de BI.";
    } else{
        // Prepare a select statement
        $sql = "SELECT ID_utilizador FROM utilizador WHERE   Bi = ?";
    ...
    if(empty($username_err) && empty($Password_err) && empty($confirm_password_err) && empty($Name_err) && empty($Nif_err) && empty($Bi_err) && empty($Telefone_err)  && empty($Localidade_err) && empty($Morada_err) && empty($Codigo_Postal_err) && empty($Email_err)){

        // Prepare an insert statement
        $sql = "INSERT INTO utilizador (Username, Password, Nome, Nif, Bi, Telefone, Localidade, Morada, Codigo_Postal, Email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssss", $param_username, $param_Password, $param_Nome,  $param_Nif, $param_Bi,  $param_Telefone,  $param_Localidade,  $param_Morada,  $param_Codigo_Postal, $param_Email);
                    ...
                    <div class="profile-basic">
                        <div class="row">
                                <div class="col-md-5">
                                    ...
                                    <div class="form-group ">
                                        <label>Bi </label>
                                        <input  type="text" name="Bi" class="form-control <?php echo (!empty($Bi_err)) ? 'is-invalid' : ''; ?>" id="<?php echo $Bi; ?>">
                                    </div>
                                    <div class="form-group ">
